...
import 'package:taskjob_aliapp/core/app_color.dart';

import '../../core/method.dart';
import '../../controller/prov_languge.dart';
...
class AchivementScreen extends StatelessWidget {
    ...
    return Directionality(
      ...
      child: Scaffold(
        appBar: AppBar(
          ...
          title: FittedBox(
            child: Text(
                '${provLang.getText('title_halfMonth_report')} ${watchHalfMonthProv.initMonth}'),
          ),
          actions: [
            TextButton(
                onPressed: () {
                    ...
                    children: [
                      ...
                      Container(
                          ...
                          child: Center(
                              child: FittedBox(
                            child: Text(
                              '${provLang.getText('ahievement_half_month')} ${averachive.toStringAsFixed(1)}%',
      ...
      if (readHomeProv.progressList.isNotEmpty) {
        ...
        while (i < readHomeProv.progressList.length) {
          if (readHomeProv.progressList[i].dateProgressYear ==
              ...
              int.parse(readHomeProv.progressList[i].dateProgressToday!) < 16) {
  ...
  checkReportAchive(BuildContext context) {
    try {
      // To check if the chosen month has 15 days of achievement
      var readhomeProv = context.read<HomeProvider>();
      var readHalfMonthProv = context.read<AcheiveReportProv>();
      int i = 0;
      int report = 0;
        ...
        itemBuilder: (_, index) {
          ...
          if (readHomeProv.progressList[index].dateProgressYear ==
              ...
              readHomeProv.progressList[index].dateProgressmonth ==
                  readHalfMonthProv.initMonth &&
              int.parse(readHomeProv.progressList[index].dateProgressToday!) <
                  16) {
            return Column(
              children: [
                Container(
...
class _DropDownDateState extends State<DropDownDate> {
  ...
  Widget build(BuildContext context) {
    return TextButton(
      onPressed: () {
        // showAlertDia(context); //check toast
      },
